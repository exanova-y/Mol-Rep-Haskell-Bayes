...
data Molecule = Molecule { -- defines new type with one constructor
    atoms :: [Atom], -- list of Atom. :: means has type. 
    bonds :: M.Map (Integer, Integer) BondType -- a map keyed by integers to bondtype. Python: dict[tuple[int, int], BondType].
  } deriving (Show, Read) -- immutable
...
data AtomicSymbol = O | H | N | C | B | Fe | F | Cl | S | Br | P | I | Na deriving (Eq, Read, Show)
