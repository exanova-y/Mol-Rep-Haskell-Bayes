...
parseSDFContents = do
    ...
    -- Skip up to the "M  END" marker
    void $ manyTill anySingle (try $ string "M  END")

    -- Optionally parse a log value block (either logP or logS)
    mLogValue <- optional $ try $ do
        void $ manyTill anySingle (try $ string ">  <")
        _ <- choice [string "logP", string "logS"]
        void $ string ">\n"
        logLine <- manyTill anySingle (try $ string "\n")
        case reads logLine of
            [(value, "")] -> return value
            _ -> fail "Invalid log value"

    -- Skip the remainder (if any)
    manyTill anySingle eof
    return (Molecule atoms bondMatrix, maybe 0.0 id mLogValue)
